...
import (
	...
	"github.com/GustavoCaso/expensetrace/service/expense"
...
func TestExpensesHandlerSortOrder(t *testing.T) {
	logger := testutil.TestLogger(t)
	s, user := testutil.SetupTestStorage(t, logger)
	...
	tests := []struct {
		...
		{
			name: "default sort: date descending - newest first",
			url:  "/expenses",
			check: func(t *testing.T, body string) {
				if !indexBefore(body, "beta", "gamma") {
					t.Error("expected 'beta' (Jan 20) before 'gamma' (Jan 15)")
				}
				if !indexBefore(body, "gamma", "alpha") {
					t.Error("expected 'gamma' (Jan 15) before 'alpha' (Jan 10)")
				}
			},
		},
		{
			...
			url:  "/expenses?sort=date:asc",
			check: func(t *testing.T, body string) {
				if !indexBefore(body, "alpha", "gamma") {
					t.Error("expected 'alpha' (Jan 10) before 'gamma' (Jan 15)")
				}
				if !indexBefore(body, "gamma", "beta") {
					t.Error("expected 'gamma' (Jan 15) before 'beta' (Jan 20)")
				}
			},
		},
		{
			name: "sort by amount descending - least negative first",
			url:  "/expenses?sort=amount:desc",
			check: func(t *testing.T, body string) {
				// desc on negative amounts: -300 > -600 > -900
				if !indexBefore(body, "alpha", "gamma") {
					t.Error("expected 'alpha' (-300) before 'gamma' (-600)")
				}
				if !indexBefore(body, "gamma", "beta") {
					t.Error("expected 'gamma' (-600) before 'beta' (-900)")
				}
			},
		},
		{
			name: "sort by amount ascending - most negative first",
			url:  "/expenses?sort=amount:asc",
			check: func(t *testing.T, body string) {
				// asc on negative amounts: -900 < -600 < -300
				if !indexBefore(body, "beta", "gamma") {
					t.Error("expected 'beta' (-900) before 'gamma' (-600)")
				}
				if !indexBefore(body, "gamma", "alpha") {
					t.Error("expected 'gamma' (-600) before 'alpha' (-300)")
				}
			},
		},
	}
