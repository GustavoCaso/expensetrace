...
ExpenseTrace implements multiple security measures to protect your data:

### CSRF Protection

The web interface includes built-in Cross-Site Request Forgery (CSRF) protection using Go 1.25's `http.CrossOriginProtection` middleware. This protection:

- Automatically rejects non-safe cross-origin browser requests
- Checks the `Sec-Fetch-Site` header (supported by all modern browsers since 2023)
- Falls back to comparing the `Origin` header with the `Host` header
- Allows safe HTTP methods (GET, HEAD, OPTIONS) from any origin
- Blocks potentially malicious cross-origin POST, PUT, and DELETE requests
