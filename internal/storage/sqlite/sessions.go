...
func (s *sqliteStorage) CreateSession(
	...
	return domain.NewSession(sessionID, userID, expiresAt, createdAt), nil
}

func (s *sqliteStorage) GetSession(ctx context.Context, sessionID string) (domain.Session, error) {
	...
	return domain.NewSession(id, userID, time.Unix(expiresAt, 0), time.Unix(createdAt, 0)), nil
}
